...
def get_urls_in_md_file_and_generate(md_file: str, re_obj_list, re_obj_for_gallery_tag, re_obj_for_pic_tag, bak_file, url_list, pool_for_write_file,threading_list,file_to_w,lock):
...
def main():
    ...
    re_obj_for_gallery_tag = re.compile(
        r'\{\s*%\s*gallery([\s\S]*?)endgallery\s*%\s*\}')
    ...
    with open('./pic.conf', 'w', encoding='utf8') as file_to_w:
        ...
        for filename in os.listdir(f'{blog_md_file_dir}'):  # 读取每个md文件
            ...
            pool.submit(get_urls_in_md_file_and_generate, md_file, [re_obj_for_link_tag, re_obj_for_image_tag, re_obj_for_headimg_tag],
                        re_obj_for_gallery_tag, re_obj_for_pic_tag, bak_file,url_list,pool_for_write_file,threading_list,file_to_w,lock)  # 读取md文件后写入custom.conf
